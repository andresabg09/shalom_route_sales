...
(campo interno, no pensado para tocarse a mano fuera de este wizard):
un cliente sale de la cola cuando se le guarda una coordenada real
(vía "Usar esta opción") o cuando se elige "Dejar como está" a
propósito -- "Borrar coordenada actual" es la única decisión que NO
saca al cliente de la cola, justamente porque el objetivo ahí es
seguir buscándole una coordenada.

Se puede abrir de dos formas:
- Desde el menú, sin nada seleccionado: carga las primeras
  LIMITE_UBICACIONES_POR_CORRIDA ubicaciones activas no revisadas.
- Desde la lista de Ubicaciones de Servicio, con una selección hecha
  ...
  solo esas ubicaciones, sin importar si ya estaban revisadas.
- Con el selector de Ruta (opcional) + botón "Cargar ubicaciones de
  esta ruta": para atender ruta por ruta en el orden que se prefiera.
"""
import logging
import math
import os
...
class ShalomBuscarGpsWizard(models.TransientModel):
    ...
    route_id = fields.Many2one(
        ...
        help="Si elegís una ruta y apretás 'Cargar ubicaciones de esta "
        "ruta', la lista se reemplaza por los clientes de ESA ruta "
        "todavía no revisados -- para poder ir ruta por ruta en el "
        "orden que prefieras.",
    )
    ...
    def action_cargar_ubicaciones(self):
        """Botón 'Cargar ubicaciones de esta ruta': reemplaza la lista
        actual por los clientes de route_id todavía no revisados (o,
        si no elegiste ninguna ruta, la selección general sin filtrar
        por ruta)."""
        self.ensure_one()
        domain = [("x_gps_wizard_revisado", "=", False)]
        if self.route_id:
        ...
        _logger.info(
            "Wizard de GPS: cargadas %s ubicación(es) de la ruta '%s'.",
...
class ShalomBuscarGpsWizardLine(models.TransientModel):
    ...
    def action_dejar_como_esta(self):
        ...
        lo revise de nuevo a mano)."""
        self.ensure_one()
        self.location_id.x_gps_wizard_revisado = True
        return _notificacion(_("'%s' no va a volver a aparecer.") % self.location_id.name)
